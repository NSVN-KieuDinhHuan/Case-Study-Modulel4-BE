...
public interface IDepositRepository extends JpaRepository<Deposit, Long> {
    ...
    @Query(value = "select deposit.id, deposit.amount, deposit.date, deposit.note, deposit.wallet_id from deposit join wallets on deposit.wallet_id = wallets.id where wallets.user_id = ?1 and (date(deposit.date) between (?2) and (?3));", nativeQuery = true)
    Iterable<Deposit> findAllDepositByUserAndTime(Long inputUser_id, String startDate, String endDate);

    @Query(value = "select deposit.id, deposit.amount, deposit.date, deposit.note, deposit.wallet_id from deposit join wallets on deposit.wallet_id = wallets.id where wallets.id = ?1 ", nativeQuery = true)
    Iterable<Deposit> findAllDepositByWallet(Long inputWallet_id);

    @Query(value = "select deposit.id, deposit.amount, deposit.date, deposit.note, deposit.wallet_id from deposit join wallets on deposit.wallet_id = wallets.id where wallets.id = ?1 and (date(deposit.date) between (?2) and (?3);", nativeQuery = true)
